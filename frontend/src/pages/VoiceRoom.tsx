  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-background">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute left-1/2 top-1/3 h-[500px] w-[700px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-accent/8 blur-[120px]" />
      </div>
      ...
      <header className="relative z-10 flex items-center justify-between border-b border-border px-4 py-4 md:px-8">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-text">Talk with {ASSISTANT_NAME}</span>
            {sessionActive && (
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <button
            ...
            className="flex items-center gap-2 rounded-full bg-app-surface px-3 py-2 text-sm text-muted transition-colors hover:bg-app-surface-hover hover:text-text sm:px-4"
          >
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-4 py-6 md:px-8 md:py-10">
        <div
          className={`grid gap-8 ${showTranscript ? 'lg:grid-cols-2 lg:items-start lg:gap-10' : 'grid-cols-1 place-items-center'}`}
        >
          <div className={`flex w-full justify-center ${showTranscript ? '' : 'min-h-[calc(100vh-11rem)] items-center'}`}>
            <JaneAssistPanel
          ...
          <AnimatePresence mode="popLayout">
            {showTranscript && (
              <motion.div
                ...
                className="glass w-full rounded-3xl p-6 md:p-8"
        ...
        {sessionActive && !micError && !speechSupported && (
          <p className="mx-auto mt-6 max-w-md text-center text-xs text-amber-400/80">
            Live speech recognition is unavailable in this browser. Use Chrome or Edge for
            full voice chat.
          </p>
        )}
      </main>
